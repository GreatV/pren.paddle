...
class Tester(object):
    ...
    def vert_val(self):
        ...
        progress = ProgressBar(
            widgets=widgets, maxval=10 * len(self.testloader)
        ).start()
        n_correct = 0.0
        n_ims = 0
        with paddle.no_grad():
            for step, (
                ...
                is_vert,
                imgpath,
            ) in enumerate(self.testloader):
                logits = self.model(ims)
                if is_vert[0]:
                    ims_counter = ims_counter
                    ...
                    score_counter, pred_counter = (
                        logits_counter[0].log_softmax(1).max(1)
                    )
