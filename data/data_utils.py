...
class Augmenter(object):
    def __init__(
        ...
        self.aug_perspective = iaa.PerspectiveTransform(
            scale=perspective, fit_output=False
        )
        self.aug_dq = iaa.Sequential(
            [iaa.Resize(resize), iaa.JpegCompression(compression)]
        )
    ...
    def apply(self, img, text_len):
        ...
        if random.random() < self.p:
            img = np.invert(img)
        if random.random() < self.p and min([h, w]) >= self.th:
...
def curve(img, r=1.0, direction=0):
    h, w, c = img.shape
    background = (
        img[0, 0].astype(np.int32)
        + img[0, w - 1].astype(np.int32)
        + img[h - 1, 0].astype(np.int32)
        + img[h - 1, w - 1].astype(np.int32)
    ) / 4
    background = background.astype(np.uint8)
    dst = np.tile(background, (int(h * (1 + r)), w, 1))
    shift = int(h * r / 2 - 1e-09)
    for j in range(w):
